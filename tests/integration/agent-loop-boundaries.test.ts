...
describe('Agent Loop 跨层边界', () => {
  it('写入失败后跳过后续调用并把两种错误交给模型', async () => {
    ...
    const client = scripted([[start, { type: 'tool_calls', calls }, done], [start, { type: 'text_delta', delta: '已收到失败反馈。' }, done]]);
    ...
    const events = await collect(manager.submit({ content: '修改' }));
  ...
  it('累计接受 100 次调用后仅发送一次无工具最终收尾请求', async () => {
    ...
    scripts.push([start, { type: 'text_delta', delta: '已达到调用上限并完成总结。' }, done]);
    ...
    const events = await collect(manager.submit({ content: '大量读取' }));
  ...
  it('跨用户轮次回放完整工具历史，并持续把恶意观察约束为不可信数据', async () => {
    ...
    const client = scripted([
      ...
      [start, { type: 'text_delta', delta: '第一轮完成。' }, done],
      [start, { type: 'text_delta', delta: '第二轮看到完整历史。' }, done],
    ]);
    const store = new InMemoryConversationStore();
    const manager = createManager(client, store, [readDefinition], async (request) => success(request, { content: malicious }));
    await collect(manager.submit({ content: '第一问' }));
    await collect(manager.submit({ content: '第二问' }));
    expect(client.requests[2]?.messages.map((message) => message.role)).toEqual(['user', 'assistant', 'tool', 'assistant', 'user']);
...
function call(index: number, name: string): ToolCallRequest { return { callId: `c${index}`, providerCallId: `p${index}`, name, input: {} }; }
