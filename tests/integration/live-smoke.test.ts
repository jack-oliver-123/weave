...
describe('live smoke 汇总', () => {
  it('只汇总脱敏元数据、两轮片段数、终态和 usage', async () => {
    const client = new FakeLlmClient(profile, [1, 2].map(() => [
      { event: { type: 'stream_start' } },
      { event: { type: 'text_delta', delta: '不应记录的回答一' } },
      { event: { type: 'text_delta', delta: '不应记录的回答二' } },
    ...
    expect(summary.turns).toEqual([
      { turn: 1, fragments: 2, status: 'completed', usage: { inputTokens: 3, outputTokens: 2 } },
      { turn: 2, fragments: 2, status: 'completed', usage: { inputTokens: 3, outputTokens: 2 } },
    ]);
