...
def validate_layout() -> None:
    ...
    is_link = link.is_symlink() or (hasattr(link, "is_junction") and link.is_junction())
    if not is_link or link.resolve() != (REPO_ROOT / "openspec").resolve():
        raise WikiSyncError("docs/openspec must link to ../openspec (symlink or Windows junction)")
...
export default defineConfig({
  lang: "zh-CN",
  title: "CloudOps WIKI",
  description: "CloudOps 项目文档与 OpenSpec 变更记录",
  cleanUrls: true,
  themeConfig: {
    nav: [
      { text: "首页", link: "/" },
