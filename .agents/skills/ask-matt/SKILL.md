...
## Weave project routing

When this skill runs in Weave, read `docs/agents/openspec-workflow.md` before choosing a route. It is the sole authority for change classification, planning artifacts, implementation, review, verification, and delivery authorization. Use the engineering map below only where it does not conflict with that project workflow.
...
3. **Hand the work to the project workflow.** It decides whether planning artifacts, tickets, or direct implementation come next. Preserve its canonical source instead of creating a parallel specification.
...
Keep steps 1–3 in **one unbroken context window** until the selected project workflow reaches its explicit handoff to implementation. That workflow defines the boundary; do not redefine it here. An implementation session can then start fresh from the context selected there.

The limit on this is the **[smart zone](https://www.aihero.dev/ai-coding-dictionary/smart-zone)**: the window (~150k tokens on state-of-the-art models) within which the model still reasons sharply. If a session approaches it before the current phase completes, don't push on degraded — `/compact` at the nearest phase boundary and carry on (see Phase boundaries).
...
- **A huge, foggy effort — a greenfield project or a huge feature build, too big for one session** → **`/wayfinder`**, the most cognitively demanding flow here. When the way from here to the destination isn't visible yet, it charts a **shared map** of **decision tickets** on the issue tracker and resolves them one at a time — producing **decisions, not deliverables** — until the fog is pushed back and the way is clear. Where **`/grill-with-docs`** sharpens an idea you can hold in one session, wayfinder is for the idea you can't — and it's slower and denser, so save it for exactly that, never a well-scoped feature.
  ...
  When the map clears, **it hands off, it doesn't build**: give the linked decisions to the project planning workflow, which turns them into buildable execution units. Looping the map straight into implementation skips that collapse and throws the linked detail away.
