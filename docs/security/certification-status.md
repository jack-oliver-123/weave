...
| Linux namespace backend | `failed` | commit `2431e745` 的定时认证在 [2026-08-17](https://github.com/jack-oliver-123/weave/actions/runs/31992969929) 与 [2026-08-24](https://github.com/jack-oliver-123/weave/actions/runs/32688600543) 均因 namespace bootstrap 不可用而失败，且签名 Secret 缺失导致没有 artifact；修复后的 v2 尚无远端签名 `passed` 证据 |
| WSL2 namespace backend | `failed` | 当前 v2 backend bootstrap 与能力 probes 在本机约 21 秒内通过，但完整 AgentLoop-to-OS 认证超过 120 秒用例上限；2026-08-15 的 v1 本地通过结果不继承到 v2 |
